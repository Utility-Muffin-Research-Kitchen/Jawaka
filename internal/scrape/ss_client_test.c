...
static int fake_search(const char *rom_name, const char *md5_hash,
                       long file_size, int system_id, jw_ss_result *result,
                       void *userdata) {
...
static void test_unhashable_path_and_terminal_results(void) {
    ...
    for (int terminal = -2; terminal <= -1; terminal++) {
        request_trace trace = {
            .outcomes = {terminal},
            .outcome_count = 1,
        };
